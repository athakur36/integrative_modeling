...
d = read_xlsx("../data/Data_May2022_Study1_ScoresQuestions.xlsx")
#names(data) <- make.names(names(data)) # for removing special characters from colnames

#var_names <- c("IV.EFA_Awareness_Large", "IV.EFA_CogRefl_Large", "IV.EFA_Spirituality_Large", "IV.EFA_Receptivness_Large", "IV.EFA_Development_Large", "SocComp")
...
```{r}
var_names <- c("Stages", "CCI", "SWLS", "FFM", "ASTI", "Mscale", "NETI", "PNSE", "IOS", "GQ.6", "IRI.Empt", "SCS", "TIPI_O", "TIPI_C", "TIPI_E", "TIPI_A", "TIPI_S", "CRTint", "CRTcorr", "BEIS", "PCSsm", "Edu", "Age", "Meditat", "Psyched", "SocComp") # IVs



# for study2
# var_names <- c("ACT1", "ACT2", "ACT3", "ACT4", "ACT5", "ACT6", "ACT7", "FFMQ1", "FFMQ2", "FFMQ3", "FFMQ4", "FFMQ5", "FFMQ6", "FFMQ7", "FFMQ8", "FFMQ9", "FFMQ10", "NETI1", "NETI2", "NETI3", "NETI4", "NETI5", "NETI6", "NETI7", "NETI8", "NETI9", "NETI10", "NETI11", "NETI12", "PNSE1", "PNSE2", "PNSE3", "PNSE4", "PNSE5", "SCS1", "SCS2", "SCS3", "SCS4", "SCS5", "SCS6", "SCS7", "SCS8", "SCS9", "SCS10", "TIPIemotStab1", "TIPIopen1", "TIPIemotStab2", "TIPIopen2", "MAIA1", "MAIA2", "MAIA3", "MAIA4", "MAIA5", "MAIA6", "MAIA7", "MAIA8", "MAIA9", "MAIA10", "Age", "")
#summary(data)
...
for (each in c(123, 5682, 19084, 1342, 3464, 917829235, 346893457, 345724, 899572, 92585623)){
  ...
  index = createDataPartition(data$SocComp, p = 0.50, list = FALSE)
  ...
  fitall <- lm(as.formula(SocComp~.), data= explain.X)
  summary(fitall)
  ...
  sub_set <- regsubsets(SocComp~., data= explain.X, nbest = 1,
  nvmax = 25, method = "exhaustive", really.big=T)
  ...
  # Train the model
  step.model <- train(SocComp ~., data = explain.X,
                      method = "leapBackward", 
  ...
  out= capture.output(predict.models[[which.min(pred.mod.results$RMSE)]]$finalModel %>% stargazer(type = 'text',star.cutoffs = c(.05,.01,.001), title = paste(c("model:"), as.character(which.min(pred.mod.results$RMSE)), c("RMSE"), as.character(min(round(pred.mod.results$RMSE, 4)))),
            multicolumn = F,
            omit.stat = c('f','ser'), report = 'vc*', no.space = T,
            order = c('1$')))
cat(paste(out, "\n"), file = "./output/awakening_reg_SocComp.txt", append= TRUE)
